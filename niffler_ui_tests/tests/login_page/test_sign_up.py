...
class TestSignUp:
    ...
    def test_user_persisted_in_db_after_successful_signup(self, login_page: LoginPage, auth_niffler_db: DbManager, delete_user, **fake):

        register_user(login_page, fake["username"], fake["password"])
        with allure.step(f"Добавить {fake['username']} в список на удаление после теста"):
            delete_user.update({"username": fake["username"]})

        with allure.step("Проверить, что запись с новым username появилась в таблице users"):
            resp = auth_niffler_db.use_table('user').filter_by({'username': fake['username']})
            AssertDB.db_contains(resp, {"username": fake['username']})

    @allure.story("Валидация username")
    @allure.title("Имя пользователя уникально (нельзя зарегистрировать дважды)")
    @FakeGenerator.username
    @FakeGenerator.password(min_len=8, max_len=12)
    def test_username_must_be_unique(self, login_page: LoginPage, auth_niffler_db: DbManager, delete_user, **fake):
        register_user(login_page, fake["username"], fake["password"])
        with allure.step(f"Добавить {fake['username']} в список на удаление после теста"):
            delete_user.update({"username": fake["username"]})
        ...
        with allure.step("Повторная регистрация с тем же username"):
            register_user(login_page, fake["username"], fake["password"])
        with allure.step("Проверить, что в базе данных нет дубликатов username"):
            resp = auth_niffler_db.use_table('user').count({'username': fake['username']})
    ...
    def test_password_stored_as_hash(
        self, login_page: LoginPage, auth_niffler_db: DbManager, delete_user, **fake
    ):
        register_user(login_page, fake["username"], fake["password"])
        with allure.step(f"Добавить {fake['username']} в список на удаление после теста"):
            delete_user.update({"username": fake["username"]})
    ...
    @FakeGenerator.password(min_len=8, max_len=12)
    def test_no_db_entry_after_failed_signup(self, login_page: LoginPage, auth_niffler_db: DbManager, delete_user, **fake):
        register_user(login_page, fake["text"], fake["password"])
        with allure.step("Проверить, что отсутствует запись после ошибки регистрации"):
            resp = auth_niffler_db.use_table('user').count({'username': fake['text']})
